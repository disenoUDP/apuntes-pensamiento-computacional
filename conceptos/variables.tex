\documentclass[../principal.tex]{subfiles}
\graphicspath{{\subfix{../imagenes/}}}

\begin{document}

\chapter{Variables}

Las variables son espacios de almacenamiento de información.

\section{Definición}

\section{Sintaxis en JavaScript}

Las variables en JavaScript se declaran usando la palabra reservada $let$.

\begin{Code}
    let unaVariable = 10;
    let otraVariable = "sopaipilla";
\end{Code}

Hasta el año tiempo las variables se declaraban usando la palabra reservada $var$, pero esta forma de declarar variables tenía problemas, por lo que se introdujo la palabra reservada $let$ en su reemplazo.

\end{document}


